...
impl WebSocketClient {
    ...
    /// Uses "all" target to set all fan zones at once (IPMI zone-based control).
    async fn set_all_fans_to_speed(&self, speed: u8) -> Result<()> {
        match self.hardware_monitor.set_fan_speed("all", speed).await {
            Ok(_) => {
                info!("All fans set to {}%", speed);
            }
            Err(e) => {
                error!("Failed to set all fans to {}%: {}", speed, e);
            }
        }
        Ok(())
    }
